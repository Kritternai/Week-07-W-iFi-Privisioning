...
static const char *TAG = "LAB7_4_SECURITY_CUSTOM";
...
typedef enum {
    LED_BLE_OFF = 0,
    LED_BLE_ADVERTISING,     // Slow Blink: ติด 500ms ดับ 500ms
    LED_BLE_CONNECTED        // Fast Blink: ติด 100ms ดับ 100ms
} led_ble_mode_t;

static volatile led_sta_mode_t g_led_sta_mode = LED_STA_OFF;
static volatile led_ble_mode_t g_led_ble_mode = LED_BLE_OFF;
                                   ...
                                   uint8_t **outbuf, ssize_t *outlen, void *priv_data)
{
    ...
    // สร้างข้อความตอบกลับแบบไดนามิกบน Heap (Protocomm จะ Free ให้อัตโนมัติหลังส่ง)
...
/* FreeRTOS Background Task สำหรับควบคุมไฟสถานะ LED 1 และ LED 2 */
static void led_status_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (g_led_ble_mode == LED_BLE_ADVERTISING) {
            gpio_set_level(LED_PIN_BLE_PROV, (tick % 10 < 5) ? 1 : 0);
        } else if (g_led_ble_mode == LED_BLE_CONNECTED) {
            gpio_set_level(LED_PIN_BLE_PROV, (tick % 2 < 1) ? 1 : 0);
        } else {
    ...
    if (event_base == PROTOCOMM_SECURITY_SESSION_EVENT) {
        ...
        switch (event_id) {
            case NETWORK_PROV_START:
                ...
                g_led_ble_mode = LED_BLE_ADVERTISING;
                break;
            case NETWORK_PROV_WIFI_CRED_RECV: {
                ...
                ESP_LOGI(TAG, "=================================================");
                g_led_ble_mode = LED_BLE_CONNECTED;
                break;
            }
...
void app_main(void)
{
    ...
    if (!provisioned) {
        ...
        // 2. ผูกฟังก์ชัน Callback เข้ากับ Endpoint หลัง Start Service
        ESP_ERROR_CHECK(network_prov_mgr_endpoint_register("custom-data", custom_prov_data_handler, NULL));
        ESP_LOGI(TAG, "Custom Endpoint 'custom-data' registered successfully!");

        ESP_LOGI(TAG, "--------------------------------------------------");
        ESP_LOGI(TAG, "[QR CODE URL]: Click or copy URL to scan QR Code:");
